...
class AccountUpdateToMultiSigKey extends Component {
  ...
  componentDidMount() {
    caver = new Caver(
      new Caver.providers.HttpProvider(URLMAP.network[this.state.network]['rpc'])
    )
  }

  handleNetworkChange = (e) => {
    caver = new Caver(
      new Caver.providers.HttpProvider(URLMAP.network[e.target.value]['rpc'])
    )
    this.setState({
      ...
      <Container>
        <Card>
          ...
          <CardBody>
            ...
            <p style={{ color: '#6c757d' }}>
              ...
              will be used for sending a AccountUpdate transaction to make the
              update to the key stored on the network, so it must possess
              sufficient KLAY.
            </p>
            <Row>
              <Col md="4">
        ...
        <Card>
          <CardHeader>
            ...
            <p style={{ color: '#6c757d' }}>
              Set threshold and the weight of each private key. Then send
          ...
          <CardBody>
            ...
            <Row>
              <Col md="8">
                <Button
                  disabled={accountUpdateButtonDisabled}
                  onClick={(e) => this.accountUpdate(e)}
                >
                  Account Update
                </Button>
              </Col>
              ...
              <Row>
                <Col md="8">
                  <CardText style={{ color: '#c221a9' }}>
                    {accountUpdateMsg}{' '}
                    {txHash !== '' && (
                      <a href={URLMAP.network[network]['finder'] + txHash}>
                        {txHash}
                      </a>
                    )}
                  </CardText>
