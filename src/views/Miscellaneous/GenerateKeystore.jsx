...
class RoleBasedKey extends Component {
  ...
  handleNumberChange = (e, idx) => {
    ...
    keys[idx] = numOfKeys[idx]
      ? new Array(numOfKeys[idx]).fill('')
      : new Array(1).fill('')
  ...
  generateRoleBasedKeys = () => {
    const roleBasedKeys = caver.wallet.keyring.generateRoleBasedKeys(
      this.state.numOfKeys
    )
    this.setState({
      keys: roleBasedKeys,
    })
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Row>
          <Col md="4">
            <Button onClick={this.generateRoleBasedKeys}>
              Generate All Keys
            </Button>
          </Col>
        </Row>
      ...
      <div>
        ...
        {keys.length > 0 && (
          <div>
            <Row>
              <Col md="4">
                <Button onClick={this.generateMultipleKeys}>
                  Generate All Keys
                </Button>
              </Col>
            </Row>
...
class GenerateKeystore extends Component {
  ...
  getKeystore = () => {
    ...
    if (isCheckedList[0]) {
      ...
      return <MultipleKey ref={this.child} setPrivateKey={this.setPrivateKey} />
    } else if (isCheckedList[2]) {
      return (
        <RoleBasedKey ref={this.child} setPrivateKey={this.setPrivateKey} />
      )
    }
  }
  ...
  downloadFile = (e) => {
    ...
    element.setAttribute(
      'href',
      'data:text/json;charset=utf-8,' + encodeURIComponent(keystore)
    )
        ...
        <Card>
          <CardHeader>
            ...
            <p style={{ color: '#6c757d' }}>
              ...
              while other keyring types are encrypted in v4 format. For more
          ...
          <CardBody>
            ...
            <Row>
              <Col md="8">
                <Button
                  style={{ display: isCheckedList[0] ? 'inline' : 'none' }}
                  onClick={this.generateKeystoreV3}
                >
            ...
            <Row>
              <Col md="12">
                <textarea
                  style={{
                    display: keystoreShown ? 'inline' : 'none',
                    height: '120px',
                  }}
                  value={keystore}
                  readOnly
            ...
            <Row>
              <Col md="8">
                <Button
                  style={{ display: keystoreShown ? 'inline' : 'none' }}
