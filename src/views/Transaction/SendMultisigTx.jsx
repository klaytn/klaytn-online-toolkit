...
class SendAndSignTx extends Component {
  ...
  onFileAndPasswordUpload = (e) => {
    ...
    const { privateKeyList, keystoreFileName, keystoreJSON, keystorePassword } =
      this.state
    try {
  ...
  onSendTxButtonClick = async (e) => {
    try {
      ...
      const vtReceipt = await caver.rpc.klay.sendRawTransaction(
        this.state.rawTransaction
      )
      this.setState({
  ...
  onSignTxButtonClick = async (e) => {
    ...
    try {
      const { sender, recipient, tokenAddress, amount, isTokenTypeSelected } =
        this.state
      this.setState({
        buttonDisabled: true,
        sendAndSignMsg: null,
        ...
        <Card>
          ...
          <CardBody>
            ...
            <p style={{ color: '#6c757d' }}>
              ...
              address, and KLAY amount. If you'd like to transfer KIP-7 or
              ERC-20 token, enter the contract address and token amount instead
              of KLAY amount.
            </p>
            <Row>
              <Col md="4">
            ...
            <Row>
              <Col md="8">
                <Button
                  ...
                  onClick={this.onSignTxButtonClick}
                >
                  Sign Transaction
                </Button>
                <Button
                  disabled={
                    buttonDisabled || this.state.rawTransaction === null
                  }
                  onClick={this.onSendTxButtonClick}
                >
                  Send Transaction
                </Button>
              </Col>
            ...
            <Row>
              <Col md="8">
                <CardText
                  style={{
                    display:
                      sendAndSignMsg != null && txHash == null
                        ? 'inline'
                        : 'none',
                ...
                >
                  {sendAndSignMsg} Transaction Hash:{' '}
                  <a href={networkLinks[network]['finder'] + txHash}>
                    {txHash}
                  </a>
                </CardText>
              </Col>
        ...
        <Card>
          <CardHeader>
            ...
            <p style={{ color: '#6c757d' }}>
              ...
              files to be used for private keys. Once decryption succeeds, you
              can see filename added in decrypted keystore list below.
            </p>
          </CardHeader>
          <CardBody>
            <Row>
            ...
            <Row>
              <Col md="8">
                <Button onClick={(e) => this.onFileAndPasswordUpload(e)}>
                  Decrypt
                </Button>
              </Col>
            </Row>
                  ...
                  (_, index) =>
                    privateKeyList[index]['key'].length > 0 && (
                      <Row>
                        ...
                        <Button
                          onClick={() => this.handleKeystoreRemove(index)}
                        >
                          Remove
                        </Button>
                      </Row>
